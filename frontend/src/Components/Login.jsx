...
export default function Login() {
  ...
  async function sendDataToLogin(values) {
    ...
    let { data } = await axios
      .post(`${import.meta.env.VITE_APP_URL}/api/user/login`, values)
      .then((response) => {
        return response;
      })
      .catch((err) => {
          ...
          >
            <div className="col-md-5">
              <div className="bg-login text-white h-100 d-flex align-items-center justify-content-center flex-column p-5 text-center">
                ...
                <p>Register Here</p>
                <Link to={"/register"}>
                  <button className="btn btn-outline-light red-font fw-bold rounded py-2 px-4">
                    Register
                  </button>
                </Link>
              </div>
